...
-- Step 3: Add budget fields to phase_tasks
ALTER TABLE phase_tasks ADD COLUMN IF NOT EXISTS allocated_budget NUMERIC(15,2) DEFAULT 0;
ALTER TABLE phase_tasks ADD COLUMN IF NOT EXISTS spent_budget NUMERIC(15,2) DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS budget_notifications (
  ...
  task_id UUID REFERENCES phase_tasks(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_budget_notifications_task ON budget_notifications(task_id);
