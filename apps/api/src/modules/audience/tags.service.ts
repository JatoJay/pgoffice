...
export class TagsService {
  ...
  async list() {
    const { rows } = await this.db.query<TagRow>(
      "SELECT id, tenant_id, name, color, created_at, updated_at FROM tags ORDER BY name ASC"
    );
    return rows;
  }

  async create(input: { tenant_id?: string | null; name: string; color?: string | null }) {
    const tenantId = this.resolveTenantId(input.tenant_id);
    const { rows } = await this.db.query<TagRow>(
      "INSERT INTO tags (tenant_id, name, color) VALUES ($1, $2, $3) RETURNING id, tenant_id, name, color, created_at, updated_at",
